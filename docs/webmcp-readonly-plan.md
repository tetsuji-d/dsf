...
- staging反映済み：段落書き込みと既存改行保持（`723d098`）。利用者のChrome内ChatGPTで取得→置換→再取得が成功したとの画面報告あり。表示上はLocal保存済み。クラウド保存や各AIへの一般的な対応を証明するものではない。
- staging `43de14d` 反映済み：原稿一覧取得、エラーの対処案内、登録数と直近の呼び出し結果表示。詳細は末尾。
- 以下の日付付き検証記録は、その時点の範囲・結果。初期の「2ツール」「書き込み未実装」は履歴として残す。stagingは読み取り3ツール、書き込み許可時5ツール。ローカルの工程1拡張は編集許可時8ツール。
...
## 原稿の発見と診断の整備（2026-09-17、ローカル）

`dsf_list_flow_paragraphs` を読み取り許可に追加。検索語がなくても現在のFlow・表示中の本文言語の見出しと段落を順番に取得できる。画像・改ページは対象外。段落追加・削除などの書き込み範囲は拡張しない。

- 入力はcontextで得た`workToken`と、続きの取得時のみ`cursor`。
- 出力は`groupId`、`languageKey`、`items`、`total`、`nextCursor`。各項目にsectionId/blockId/type、1始まりのindex、最大80書記素のexcerpt、UTF-16のtextLength、empty/missingTranslationを返す。
- 1回最大20件。返されたnextCursorを順に1回ずつ使い、nullなら末尾。新しい先頭取得は以前のcursorを置き換える。原稿・対象Flow・表示言語が変わった場合や古いcursorはSTALE_CURSOR。cursorなしで取り直す。作品切替・許可解除で失効する。
- 翻訳が未入力なら空文字。原文への代替表示はしない。空の段落もIDを返すため、書き込み許可後の全文取得・入力へつなげられる。
- contextには作品の`languageKeys`を追加。原稿本文・注釈・例外メッセージを診断情報に含めない。

通常のツールエラーは読み取り・書き込みとも`{error:{code,recovery}}`。recoveryは静的な英語の対処案内。BUSYなら入力・組版待ち、古い編集券なら再取得、改行変更なら既存改行の保持を案内する。解除済みのコールバックや中断要求は従来どおり実行を拒否する。

プロフィールには登録数（読み取り3／編集許可時5）と`document.modelContext`、直近1回のツール名・成功/本文更新/再送/エラーの結果を表示する。登録成功だけでAI接続成功とは表示しない。呼び出し元AIは特定しない。対処案内はUI言語のJA/ENに対応。結果はタブのメモリ内のみ、連携解除や作品切替で消去する。

検証：readonly/write/Flow置換/Flow検索の回帰テストとstagingビルドが通過。一覧の26件のページング、空段落、未入力翻訳、書記素境界、古いcursor、busy、原稿不変を検証。Chrome実APIと合成原稿の検証画面で、3/5ツール表示、一覧取得、期限切れトークンと対処案内、改行保持の更新、同一更新の再送（履歴追加なし）、Undo、連携オフを確認した。今回の追加ツールをChrome内ChatGPTが自動選択することは未検証。
