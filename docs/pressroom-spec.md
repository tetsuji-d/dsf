...
ユーザーはここでプロジェクトを選択してレンダリングし、公開ステータスを管理する。
...
4. ステータスを `draft` で作成。その後ユーザーがステータスを変更して公開
...
│   ├── レンダリング (.dsp → .dsf)
│   └── 発行（ステータス: draft で Works Room に登録）
│
...
- **非破壊** — DSP ファイル自体は変更しない。DSP を読み込んでDSFを生成するだけ
