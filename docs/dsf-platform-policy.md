...
Web提供、Horizon、Viewer、portable DSF、PWA、版管理、WebMCP連携についての製品方針を一元化する。
...
## 9. WebMCPを前提とする機能設計（確定方針・接続は今後の実装）

2026-09-12、ユーザーは今後の機能をWebMCP導入前提で実装する方針を決定した。
新規・改修する編集機能は、UIとAIから同じ編集処理を呼べるように設計する。
これは全機能の一括改修や、各AI製品での接続完了を意味しない。

### 接続の位置づけ

- WebMCPは、開いているWebページがブラウザ内のAIエージェントへ構造化された操作を提供する入口とする。
- リモートMCPサーバーは別の接続方式。WebMCPを導入するだけでChatGPT／Claude／Gemini／Apple Intelligenceの全製品から接続できるとは扱わない。ブラウザと接続クライアントの対応は個別に検証する。
- Web中心の提供を維持する。WebMCP非対応時も、通常の編集・保存・閲覧を利用できるようにする。
- WebMCP依存は薄い接続モジュールへ集約し、編集処理へブラウザAPIの仕様を埋め込まない。接続時に機能検出し、登録・解除と画面切替時の寿命を管理する。
- 2026-09-12確認の仕様はCommunity Groupの草案であり、W3C標準ではない。API名・署名・ブラウザ提供条件は実装時に一次資料で再確認する。

### 新規・改修機能の共通設計

- UIイベントから編集コマンドを分離する。リボン、ショートカット、WebMCPは同じ検証・更新・再組版・Undo経路を呼ぶ。
- 操作対象はプロジェクト、原稿、段落、画像などの安定IDと言語で指定する。画面座標や再ページ化で変わるページ番号だけで対象を確定しない。
- 入力・結果・失敗理由を構造化し、対象範囲と変更内容を返す。実行時に現在の権限と編集可能状態を再確認する。
- 未保存内容を含むアクティブな編集セッションを正とする。原稿の更新世代を照合し、人間の並行編集を古い内容で上書きしない。
- 一括操作は検証後にまとめて適用し、Undoの単位を明確にする。再試行で二重挿入・二重置換しない契約を設計する。
- 読取、編集、保存、公開、削除の能力を分ける。編集許可から発行許可を推論しない。AIにも既存の公開・保存の権限確認を適用する。
- ツール出力は依頼に必要な範囲に限定する。認証情報を返さず、原稿やキャプション中の文章を操作指示として扱わない。
- ルビ・圏点・翻訳状態・画像の段落紐づけ・固定レイアウトを既存契約に従って維持する。WebMCPのためにDSP／DSFへ独自フィールドを追加しない。

最初の候補は、現在の編集対象の取得、原稿検索、置換案の作成と適用。
検索・置換は表示DOMではなく原稿データを対象とする共通コマンドとして設計する。
既存機能は関連箇所を改修する単位で対応し、接続試験ではUIとツールの結果一致、Undo、競合、非対応ブラウザを確認する。
WebMCP接続自体の実装・検証完了は別途記録する。commit・デプロイは既存の依頼境界に従う。

参考: [WebMCP仕様草案](https://webmachinelearning.github.io/webmcp/)、
