...
// Line breaks inside an existing semantic paragraph are content, not new blocks.
export function preservesParagraphLineBreaks(before, after) {
    const breaks = value => value.match(/\r\n|[\r\n\u2028\u2029]/gu) || [];
    return JSON.stringify(breaks(before)) === JSON.stringify(breaks(after));
}
...
export function editFlowParagraph(blocks, target, expectedText, text) {
    ...
    if (typeof text !== 'string' || text.length > 12000) fail('INVALID_TEXT');
    ...
    if (!preservesParagraphLineBreaks(expectedText, text)) fail('LINE_BREAKS_CHANGED');
