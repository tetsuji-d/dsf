...
(async()=>{const browser=await chromium.launch({channel:'chrome',headless:true});try{
 ...
 await p.evaluate(()=>window.setStudioUILang('en'));await openProfile();assert.equal(await p.locator('[data-ai-title]').first().innerText(),'AI connection');
 ...
 await p.evaluate(()=>{testTools.set('other_owner',{});testMode='fail';});await toggle().click();await p.waitForFunction(()=>document.querySelector('[data-ai-status]').textContent.startsWith('Could not connect'));
 assert.deepEqual(await p.evaluate(()=>[...testTools.keys()]),['other_owner']);
 await p.evaluate(()=>testMode='ok');await p.locator('[data-ai-retry]').first().click();await p.waitForFunction(()=>testTools.has('dsf_search_flow_text'));
 await p.evaluate(()=>window.switchRoom('home'));assert.equal(await p.evaluate(()=>testTools.has('dsf_get_editor_context')),false);
 await p.evaluate(()=>window.switchRoom('editor'));await openProfile();assert.equal(await toggle().isChecked(),true);
 await p.waitForFunction(()=>testTools.has('dsf_search_flow_text'));
 ...
 });assert.deepEqual(race,{on:'on',count:6,after:0});
