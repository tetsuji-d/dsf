...
assert.equal(await page.locator('.ribbon-command-row').isVisible(),false);
assert.equal(await page.locator('#flow-compare-split').evaluate(e=>!!e.closest('.ribbon-top-row')),true);
for(const selector of ['[onclick="insertFlowGroupBeforeActive()"]','[onclick="exportDSP()"]','#flow-ribbon-placement','#flow-ribbon-view']) {
 assert.ok(await page.locator('#ribbon-bar').locator(selector).evaluate(e=>getComputedStyle(e).display!=='none'),selector);
}
const beforeDisplay=await read();
for(const key of ['ruler','guides','line','ruled']){const b=page.locator('#flow-ribbon-view [data-ribbon-display='+key+']');const old=await b.getAttribute('aria-pressed');await b.click();assert.notEqual(await b.getAttribute('aria-pressed'),old);assert.equal(await page.locator('#flow-ribbon-view [data-ribbon-display='+key+']').getAttribute('aria-pressed'),old==='true'?'false':'true');await page.locator('#flow-ribbon-view [data-ribbon-display='+key+']').click();}
...
await page.locator('#btn-toggle-sidebar').click();await page.locator('.project-asset-card').first().waitFor({state:'visible'});await page.locator('#asset-search').fill('missing');assert.equal(await page.locator('.project-asset-card').count(),0);await page.locator('#asset-search').fill('sample');await page.locator('.project-asset-card').first().click({button:'right'});assert.equal(await page.locator('.project-asset-menu [role=menuitem]').count(),4);await page.keyboard.press('Escape');await page.locator('#btn-toggle-sidebar').click();assert.equal(await page.locator('#sidebar').isVisible(),false);
...
await page.locator('#page-strip-thumbs [data-editor-unit-id=picture]').click();assert.equal(await page.locator('#panel-right').isVisible(),false);assert.equal(await page.locator('#image-ribbon-home').isVisible(),true);assert.equal(await page.locator('#flow-ribbon-home').isVisible(),false);await edit();
