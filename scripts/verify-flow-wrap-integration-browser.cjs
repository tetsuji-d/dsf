...
(async()=>{const browser=await chromium.launch({channel:'chrome',headless:true});try{const page=await browser.newPage();await page.goto('http://127.0.0.1:5178/scripts/fixtures/flow-image-wrap-lab.html');console.log(await page.evaluate(async()=>{
 ...
 for(const language of ['ja','en-us'])for(const wrap of ['square','band']){
  ...
  group.flow.layout.schemaVersion=2;group.flow.layout.anchoredObjects=[{id:'image',anchorBlockId:'a',graphic,wrap,gapEm:.75},{id:'shape',anchorBlockId:'b',graphic:shape,wrap,gapEm:.75}];
  ...
  check(pagination.pages.filter(p=>p.anchoredObject).length===2,'one object per page');
  if(language==='ja'&&wrap==='square'){
   ...
   const tampered=structuredClone(pagination);tampered.pages.find(p=>p.anchoredObject).anchoredObject.graphic.frame.x++;
  ...
  check(manifest.pages.every(p=>p.renderKind==='fixedText'),'body is fixedText');check(manifest.pages.filter(p=>p.background?.imageHref).length===2,'both backgrounds present');
  ...
  check(handoff.imageFiles.length===2,'Horizon backgrounds missing');
