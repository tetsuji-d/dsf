...
// An unavailable exact translation continues to display read-only source fallback.
await page.locator('#lang-tabs-top .lang-tab').filter({hasText:'EN-US'}).click();await page.locator('#page-strip-thumbs [data-flow-page-index="0"]').first().click();await page.locator('.flow-dom-block').filter({visible:true}).first().click();assert.equal(await page.locator('.flow-direct-input-proxy').count(),0);assert.deepEqual((await read())[0].flow.document.sections[0].blocks.map(b=>b.texts['en-US']),[undefined,undefined]);
console.log('Missing translation cannot edit fallback source');
...
const beforeTail=await read();await page.locator('[onclick="showTailPageAddMenu(event)"]').click();await page.locator('.context-menu-item').filter({hasText:'Flow原稿を追加'}).click();await page.locator('.flow-authoring-input').first().waitFor({state:'visible'});const afterTail=await read();assert.equal(afterTail.length,beforeTail.length+1);assert.equal(afterTail.at(-1).kind,'flow');assert.deepEqual(afterTail.slice(0,-1),beforeTail);await page.locator('#btn-undo').click();assert.deepEqual(await read(),beforeTail);
...
for(const width of [1440,1280,1024]){await page.setViewportSize({width,height:800});const rect=await page.locator('#image-ribbon-home').boundingBox();assert.ok(rect.x>=0&&rect.x+rect.width<=width);}
